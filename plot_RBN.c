...
This RBN model is setup by 3 steps:
    ...
    3. Randomly give outputs for Boolean functions and each node have different set of Boolean functions
...
int main(int argc, char *argv[]){
    ...
    if (argc == 5){
        ...
        fprintf(stderr,"Usage: RBN.x N(node numbers) k(in-degree numbers) seed tmax \n");
        return 1;
    }
...
void init(){
    ...
    for (i=0; i<N; ++i){
        for (j=0; j<F; ++j){
            roll = ran2(&seed);
            if (roll < 0.5) {functions[i][j] = 1;} // unifrom distrubution between 0 to N-1 (unbias output)
