...
int main(int argc, char *argv[]){
    ...
    mnode = ivector(0,N-1);
    // Save configue after some run that try to avoid basian state
    tnode = ivector(0,N-1);
    ...
    len = 0;
    value = 0;
    memcpy(tnode, node, N*sizeof(*node)); // Copy the configuration for First attractor finding loop

    // Check for the first time
    value = length(1, &len);
    if (value == 1){fprintf(stderr,"Find attractor. Length: %d \n", len); return 0;}
    else{fprintf(stderr, "Second loop for finding attractor \n");}

    memcpy(tnode, node, N*sizeof(*node)); // Copy the configuration for Second loop
...
void init(){
    ...
    for (i=0; i<F; ++i){
        roll = ran2(&seed);
        if (roll < 0.5) {truth[i] = 1;} // unifrom distrubution between 0 to N-1 (unbias output)
        else {truth[i]=0;}
    }
