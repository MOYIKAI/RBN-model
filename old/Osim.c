...
#include "print.h"      // Saving info to ASCII files
...
int main(int argc, char *argv[]){
  ...
  int **IniS;        // populations of Initial conition of nodes
  ...
  if (argc == 14){
    ...
    fprintf(stderr,"Usage: main.x N K P(population) G(Num of Generation) seedS(Structure) seedB(Boolean) seedI(Initial states)\
      p(Probability for Boolean function) eps(Mutation Probability) attfile BooleanPfile MIfile IC&S\n");
    return 1;
  }
  ...
  saveconnect(argv[13], connect, N, K);
  GBoolfS(BoolfS, N, K, S, p, seedB);
  saveBoolfS(argv[11], BoolfS, N, K, S);
  GIniS2(IniS, N, S, seedI);
  saveIniS(argv[13], IniS, N, S);
  ...
  for (g=0; g<G; g++){
    ...
    // Pach Generation
    for (i=0; i < S; i++){
      AIP2node(IniS[i], node, N);          // IniS[i] to node
    ...
    //reset Mutual info, fitness adn rep population
    Zeros1d_int(rep, S);
    Zeros1d_dobule(MI, S);
    Zeros1d_dobule(fitness, S);
  }
