...
int main(int argc, char *argv[]){
  ...
  float p, eps, ga;  // Boolean function probability, Mutuation probability , gamma(for fitness function)
  ...
  for (g=0; g<G; g++){
    ...
    for (i=0; i<S; i++){fitness[i] = pow(MI[i],ga)/sumMI;}
    showMI(MI, S);
